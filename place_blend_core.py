...
def _soft_blend(images: list[Image.Image], size: tuple[int, int], transition: float) -> Image.Image:
    """Blend neighbouring image panels only at their seams.

    Build 2.0 resized every source across the whole canvas and cross-faded all
    landmarks on top of one another. This segmented blend keeps each place clear
    and uses a narrow feathered overlap between neighbouring sections.
    """

    w, h = size
    n = max(1, len(images))
    if n == 1:
        return _cover(images[0], size)

    segment = w / n
    overlap = int(np.clip(segment * 0.12 * float(transition), 12, max(14, segment * 0.28)))
    accum = np.zeros((h, w, 3), dtype=np.float32)
    weight_sum = np.zeros((h, w, 1), dtype=np.float32)

    for index, image in enumerate(images):
        nominal_left = int(round(index * segment))
        nominal_right = int(round((index + 1) * segment))
        left = max(0, nominal_left - (overlap if index > 0 else 0))
        right = min(w, nominal_right + (overlap if index < n - 1 else 0))
        panel_w = max(1, right - left)
        panel = _cover(image, (panel_w, h))
        arr = np.asarray(panel, dtype=np.float32)

        alpha = np.ones((h, panel_w, 1), dtype=np.float32)
        if index > 0:
            fade = min(overlap * 2, panel_w)
            alpha[:, :fade, 0] = np.linspace(0.0, 1.0, fade, dtype=np.float32)
        if index < n - 1:
            fade = min(overlap * 2, panel_w)
            alpha[:, -fade:, 0] = np.minimum(
                alpha[:, -fade:, 0],
                np.linspace(1.0, 0.0, fade, dtype=np.float32),
            )

        accum[:, left:right, :] += arr * alpha
        weight_sum[:, left:right, :] += alpha

    out = accum / np.maximum(weight_sum, 1e-6)
...
def _travel_poster(images: list[Image.Image], places: list[PlaceItem], size: tuple[int, int], transition: float,
    ...
    # Keep the artwork clear. Only soften the title area instead of washing a
    # cream gradient over the entire poster.
    top_h = max(150, min(size[1] // 4, 250))
    top_overlay = Image.new("RGBA", (size[0], top_h), (253, 246, 234, 0))
    top_overlay.putalpha(_vertical_gradient((size[0], top_h), 175, 0).getchannel("A"))
    canvas.alpha_composite(top_overlay, dest=(0, 0))

    # Restrict label darkening to a shallow lower band as well.
    bottom_h = max(150, min(size[1] // 4, 240))
    bottom = Image.new("RGBA", (size[0], bottom_h), (58, 44, 32, 0))
    bottom.putalpha(_vertical_gradient((size[0], bottom_h), 0, 95).getchannel("A"))
    canvas.alpha_composite(bottom, dest=(0, size[1] - bottom_h))
...
def _project_zip(image_bytes: bytes, settings: dict) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            ...
            "Clean reference-guided illustration may have been used to restyle the source photos.\n"
...
def _styled_sources_zip(styled_items: list[PlaceItem], dpi: int = 150) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            ...
            "The original source composition remains; the engine applies a clean illustrated finish and reference-guided colour palette.\n"
...
def _project_zip_v2(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            "README.txt",
            "Teddie & Lane Place Blend Maker Build 2.1\n\n"
            "The final poster and styled source images are included.\n"
            "The uploaded reference image is NOT copied into this ZIP.\n"
            "The clean local renderer uses OpenCV image processing only; it does not call a paid image-generation API.\n"
            "It keeps the content/layout of each source photo and applies a clean illustrated finish with reference-guided colour.\n"
...
def _quality_settings(label: str) -> tuple[int, int]:
    values = {
        "Draft — fastest": (640, 1),
        "Balanced — recommended": (900, 1),
        "Detailed — slower": (1200, 1),
    }
...
def _build_styled_items(
    ...
    *,
    ...
    for index, place in enumerate(places):
        ...
        if engine == "Clean reference-guided illustration":
...
def render_app() -> None:
    ...
    st.title("📍 Place Blend Maker — Clean Local Travel Art")
    st.caption(
    ...
    st.markdown(
        "<div class='tl-local'><b>What creates the art style now?</b> The app itself uses your uploaded "
        "reference image to guide the colour palette while its clean local renderer creates the illustrated travel-poster finish. It keeps the original landmark layout and does not invent a new fantasy scene. "
    ...
    with st.sidebar:
        ...
        engine = st.selectbox(
            "Style method",
            ["Clean reference-guided illustration", "Fast clean travel-poster finish"],
            help=(
                "Clean illustration is recommended. It preserves buildings and avoids the coloured grain from Build 2.0. "
                "Fast clean mode uses a smaller working size for lower-powered hosting."
            ),
        )
        ...
        quality_label = st.selectbox(
            ...
            index=1,
            disabled=engine != "Clean reference-guided illustration",
        )
    ...
    elif engine == "Clean reference-guided illustration":
        st.info("Upload the style image before creating the illustrated result.")
    else:
    ...
    style_ready = style_reference is not None or engine == "Fast clean travel-poster finish"
    if not complete:
        st.info(f"Upload all {int(place_count)} source photos.")
    if not style_ready:
        st.info("Upload the style-reference image for the clean illustration mode.")

    settings = {
        "format": "teddie_lane_place_blend_v2_1",
        "build": "2.1",
        "engine": engine,
        "paid_api_used": False,
    ...
    if create_clicked:
        progress = st.progress(0.0, text="Preparing clean local illustration…")
        try:
            ...
            st.error(str(exc))
            if engine == "Clean reference-guided illustration":
                st.caption("The Fast clean travel-poster finish remains available if the free server cannot run the local model.")
        except Exception as exc:
            progress.empty()
            st.exception(exc)
    ...
    if stored and stored.get("signature") == signature:
        ...
        st.success("The same clean local art settings were applied to every place.")
        ...
        st.image(final_image, caption="Consistent clean travel-art design", use_container_width=True)
    ...
    st.markdown(
        "<div class='tl-note'><b>Accuracy note:</b> The clean local renderer restyles your photographs; it does not generate "
