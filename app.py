...
def make_scenic_recipe(destinations, horizon_y, label_settings, canvas, art_settings):
    ...
    return {
        ...
        "build_name": "Multi-Style Place Blender 3.1.0",
...
# -----------------------------------------------------------------------------
# Isolated A-series print resolution verifier.
# This section intentionally does not modify destination matching, recipe
# generation, prompt building, presets, or artwork logic above.
# -----------------------------------------------------------------------------

st.divider()
with st.expander("Print Resolution Verifier (optional)", expanded=False):
    st.caption(
        "Checks the real source and final image pixels. It does not change the "
        "destination recipe or artwork prompt."
    )
    ...
    try:
        ...
        source_file = st.file_uploader(
            ...
            type=["png", "jpg", "jpeg", "webp"],
            key="isolated_a_series_source_artwork",
        )

        processing_method = st.selectbox(
            "How was the final image produced?",
            options=[
                "none",
                "ai_super_resolution",
            ...
            key="isolated_a_series_processing_method",
        )

        final_file = st.file_uploader(
            "Final enlarged artwork (optional)",
            type=["png", "jpg", "jpeg", "webp"],
            key="isolated_a_series_final_artwork",
        )

        if source_file is not None:
            result = validate_uploaded_artwork(
                source_file,
                final_image=final_file,
                processing_method=processing_method,
            )
            label = classification_label(result.classification)

            if result.classification in {"native", "ai_upscaled"}:
                st.success(label)
            elif result.classification == "standard_resized":
                st.warning(label)
            else:
                st.error(label)

            col1, col2, col3 = st.columns(3)
            col1.metric("Original pixels", f"{result.source_width_px} × {result.source_height_px}")
            col2.metric("Final pixels", f"{result.final_width_px} × {result.final_height_px}")
            col3.metric("Effective A1 DPI", f"{result.effective_dpi:.1f}")

            report_rows = [
                {"Check": "Target print", "Result": result.target_print_standard},
                {"Check": "Required pixels", "Result": f"{result.target_width_px} × {result.target_height_px}"},
                {"Check": "Original megapixels", "Result": result.source_megapixels},
                {"Check": "Final megapixels", "Result": result.final_megapixels},
                {"Check": "Processing method", "Result": result.processing_method},
                {"Check": "Classification", "Result": label},
                {"Check": "Meets 240 DPI minimum", "Result": "Yes" if result.passes_minimum_240_dpi else "No"},
                {"Check": "Meets 300 DPI target", "Result": "Yes" if result.passes_target_300_dpi else "No"},
                {"Check": "Native A1 source", "Result": "Yes" if result.native_target_met else "No"},
            ]
            ...
            st.download_button(
                ...
                mime="application/json",
                key="isolated_a_series_download_report",
            )
    except Exception as exc:
        st.error(f"The optional print verifier could not load: {exc}")
