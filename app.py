...
st.set_page_config(
    page_title="Place Blend Recipe Builder 3.2",
    page_icon="🎨",
    layout="wide",
)
...
PRINT_PRODUCTION_PRESETS = {
    "A4 Portrait": {
        "print_standard": "A4", "orientation": "portrait",
        "physical_size_mm": {"width": 210, "height": 297},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 2480, "height": 3508},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 1984, "height": 2806},
    },
    "A4 Landscape": {
        "print_standard": "A4", "orientation": "landscape",
        "physical_size_mm": {"width": 297, "height": 210},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 3508, "height": 2480},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 2806, "height": 1984},
    },
    "A3 Portrait": {
        "print_standard": "A3", "orientation": "portrait",
        "physical_size_mm": {"width": 297, "height": 420},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 3508, "height": 4961},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 2806, "height": 3969},
    },
    "A3 Landscape": {
        "print_standard": "A3", "orientation": "landscape",
        "physical_size_mm": {"width": 420, "height": 297},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 4961, "height": 3508},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 3969, "height": 2806},
    },
    "A2 Portrait": {
        "print_standard": "A2", "orientation": "portrait",
        "physical_size_mm": {"width": 420, "height": 594},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 4961, "height": 7016},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 3969, "height": 5613},
    },
    "A2 Landscape": {
        "print_standard": "A2", "orientation": "landscape",
        "physical_size_mm": {"width": 594, "height": 420},
        "target_dpi": 300,
        "target_pixel_dimensions": {"width": 7016, "height": 4961},
        "minimum_acceptable_dpi": 240,
        "minimum_pixel_dimensions": {"width": 5613, "height": 3969},
    },
    "A1 Portrait": {
        "print_standard": "A1", "orientation": "portrait",
    ...
    "A1 Landscape": {
        "print_standard": "A1", "orientation": "landscape",
...
def build_production_output(print_preset_name, bleed_mm, safe_margin_mm):
    preset = deepcopy(PRINT_PRODUCTION_PRESETS[print_preset_name])
    pixels = preset["target_pixel_dimensions"]
    megapixels = round((pixels["width"] * pixels["height"]) / 1_000_000, 2)
    preset.update({
        ...
        "generation_status": "A-series-ready source artwork",
        "native_label_rule": "Only label Native when the original generated image itself meets or exceeds the target pixel dimensions before any enlargement.",
        "resampling_rule": "Generate at the highest available native resolution. If enlargement is required, use a genuine AI super-resolution process and disclose that it is AI-upscaled. Do not only change DPI metadata.",
        "aspect_ratio_rule": "Preserve the exact ISO A-series aspect ratio. Do not stretch, squash or distort the artwork or landmark.",
        "quality_rules": [
            "exceptionally crisp edges", "large readable forms", "clean colour separation",
            "simple accurate landmark geometry", "minimal micro-detail", "minimal texture",
            "no blur", "no softness", "no noise", "no compression artefacts",
            "no tiny decorative elements", "optimised for high-quality AI enlargement",
            "keep typography sharp and correctly spelled", "retain the entire artwork canvas without cropping",
            "keep important objects inside the safe margin"
        ],
        "classification_rules": {
            "native": "Original source already meets or exceeds the selected target pixel dimensions.",
            "ai_upscaled": "Final file meets the target after genuine AI super-resolution.",
            "standard_resized": "Final file meets dimensions after ordinary interpolation only.",
            "page_only": "PDF page has the selected physical size but embedded artwork is below target resolution."
        },
        "final_validation": {
            "confirm_actual_pixel_dimensions": True,
            "confirm_target_dpi_at_selected_size": True,
            "confirm_no_stretching": True,
            "confirm_no_blurry_edges_at_100_percent_zoom": True,
            "confirm_dpi_is_not_metadata_only": True,
            "do_not_label_as_native_unless_source_matches_target_pixels": True
        },
        "native_generation": {
            "generate_at_highest_available_native_resolution": True,
            "do_not_fake_native_a1": True,
            "record_actual_generated_pixel_dimensions": True,
            "record_actual_megapixels": True,
            "record_actual_aspect_ratio": True,
            "preserve_iso_a_series_ratio": True,
            "if_native_pixels_below_target": {
                "classification": "AI_UPSCALED_REQUIRED",
                "required_final_pixels": deepcopy(preset["target_pixel_dimensions"]),
                "do_not_call_native": True
            }
        },
        "verification_engine": {
            "automatic_output_verification": True,
            "classification_priority": ["native", "ai_upscaled", "standard_resized", "page_only"],
            "require_processing_history_for_ai_upscaled_label": True,
            "require_original_source_dimensions": True
        }
    })
    return preset
...
def make_scenic_recipe(destinations, horizon_y, label_settings, canvas, art_settings):
    ...
    # Apply style-specific rendering controls from the UI.
    if style_name in ["3D Family Animation", "Classic Fairytale Animation"]:
        mood = art_settings.get("animation_mood", "Warm and joyful")
        character_presence = art_settings.get("character_presence", "Tiny background figures only")
        depth = art_settings.get("cinematic_depth", "Balanced cinematic depth")
        material = art_settings.get("material_finish", "Soft matte")
        expression = art_settings.get("facial_expression", "Friendly and expressive")
        render["finish"] += f" Mood: {mood}. Character presence: {character_presence}. Scene depth: {depth}. Surface finish: {material}. Character expression: {expression}."
        if style_name == "3D Family Animation":
            render["finish"] += " Use smooth dimensional shading, soft tactile materials and no visible paper grain, hatching or drawn linework."
        else:
            render["finish"] += " Use graceful hand-drawn forms, softly painted backgrounds and no glossy 3D material treatment."
    ...
    if style_name == "Bold Pop Mosaic":
        ...
        render["finish"] += f" Pattern strength: {pattern_strength}. Outline strength: {outline_strength}. Landmark simplification: {landmark_simplification}. Feature patterns such as {patterns}. Background: {background_style}."
        render["palette"] += f" Colour intensity: {colour_intensity}."
    ...
    return {
        "recipe_version": "1.6.0",
        "build_name": "Multi-Style Place Blender 3.2.0",
...
def make_production_prompt(recipe):
    ...
    return f"""
A-SERIES PRINT-PRODUCTION REQUIREMENTS:
...
- Preserve the exact ISO A-series aspect ratio; do not stretch, squash or distort the artwork.
...
- Before export, verify actual pixel dimensions, selected physical dimensions, sharpness at 100% zoom, correct spelling and no cropping.
"""
...
st.title("🎨 Place Blender 3.1 — A-Series Print Engine")
st.caption("Upgrade 1 — exact A4/A3/A2/A1 production targets")
st.write(
...
4. Choose an A-series portrait or landscape source proportion, then select the required print size.
5. Press **Build artwork recipe and prompt**.
...
format_name = st.radio(
    ...
    ["A-Series Landscape", "A-Series Portrait"],
    horizontal=True,
)
# A-series generation masters use the ISO 216 ratio rather than 3:2 / 2:3.
if format_name == "A-Series Landscape":
    canvas_settings = {
        "orientation": "landscape", "format_description": "ISO A-series landscape poster master",
        "ratio": "1.4142:1", "width": 2896, "height": 2048,
        "default_horizon_y": 1120, "horizon_min": 840, "horizon_max": 1400,
    }
else:
    canvas_settings = {
        "orientation": "portrait", "format_description": "ISO A-series portrait poster master",
        "ratio": "1:1.4142", "width": 2048, "height": 2896,
        "default_horizon_y": 1695, "horizon_min": 1320, "horizon_max": 2150,
    }
...
title_text = ""
subtitle_text = ""
st.subheader("A-series print output")
orientation = canvas_settings.get("orientation")
size_name = st.selectbox("Print size", ["A4", "A3", "A2", "A1"], index=3,
    help="Adds exact physical dimensions, 300 DPI pixel targets and honest native/upscaled labelling rules.")
print_preset_name = f"{size_name} {orientation.title()}"
selected_print = PRINT_PRODUCTION_PRESETS[print_preset_name]
target_px = selected_print["target_pixel_dimensions"]
target_mm = selected_print["physical_size_mm"]
target_mp = target_px["width"] * target_px["height"] / 1_000_000
col_a, col_b, col_c = st.columns(3)
col_a.metric("Physical size", f"{target_mm['width']} × {target_mm['height']} mm")
col_b.metric("300 DPI pixels", f"{target_px['width']:,} × {target_px['height']:,}")
col_c.metric("Target megapixels", f"{target_mp:.2f} MP")
...
# ── SCENIC POSTER / RETRO / WATERCOLOUR QUESTIONS ─────────────────────────
    ...
    if style_name in ["3D Family Animation", "Classic Fairytale Animation"]:
        st.subheader("3. Animation-style options")
        ...
        if style_name == "3D Family Animation":
            material_options = ["Soft matte", "Polished but not glossy", "Soft tactile", "Cinematic painted 3D"]
        else:
            material_options = ["Luminous painted", "Soft storybook texture", "Classic cel-painted", "Elegant hand-painted"]
        material_finish = st.selectbox("Surface finish", material_options, index=0, key="animation_material")
...
# ── VERSION 3.2: HONEST PRINT-RESOLUTION VERIFICATION ──────────────────────
from resolution_validator import classification_label, validate_uploaded_artwork

st.divider()
st.header("Print Resolution Verifier")
st.caption(
    "Check the real source pixels and the final enlarged file before calling artwork Native, AI Upscaled, Standard Resized, or Page Only."
)

verify_col1, verify_col2 = st.columns(2)
with verify_col1:
    verifier_source = st.file_uploader(
        "Original generated artwork",
        type=["png", "jpg", "jpeg", "webp"],
        key="v32_verifier_source",
        help="Upload the untouched image originally produced by the image generator."
    )
with verify_col2:
    verifier_final = st.file_uploader(
        "Final enlarged artwork (optional)",
        type=["png", "jpg", "jpeg", "webp"],
        key="v32_verifier_final",
        help="Upload the finished enlarged file when one exists."
    )

verifier_method = st.selectbox(
    "How was the final file produced?",
    ["none", "ai_super_resolution", "standard_interpolation", "metadata_only", "unknown"],
    format_func=lambda value: {
        "none": "No enlargement — original file",
        "ai_super_resolution": "Genuine AI super-resolution",
        "standard_interpolation": "Ordinary resize/interpolation",
        "metadata_only": "DPI metadata only",
        "unknown": "Unknown",
    }[value],
    key="v32_processing_method"
)

if verifier_source is not None:
    try:
        verifier_result = validate_uploaded_artwork(
            verifier_source,
            final_image=verifier_final,
            processing_method=verifier_method,
        )
        verifier_label = classification_label(verifier_result.classification)

        if verifier_result.classification in {"native", "ai_upscaled"}:
            st.success(verifier_label)
        elif verifier_result.classification == "standard_resized":
            st.warning(verifier_label)
        else:
            st.error(verifier_label)

        metric1, metric2, metric3, metric4 = st.columns(4)
        metric1.metric("Original pixels", f"{verifier_result.source_width_px:,} × {verifier_result.source_height_px:,}")
        metric2.metric("Final pixels", f"{verifier_result.final_width_px:,} × {verifier_result.final_height_px:,}")
        metric3.metric("Effective A1 DPI", f"{verifier_result.effective_dpi:.1f}")
        metric4.metric("Final megapixels", f"{verifier_result.final_megapixels:.2f} MP")

        report_rows = {
            "Target print": verifier_result.target_print_standard,
            "Required pixels": f"{verifier_result.target_width_px:,} × {verifier_result.target_height_px:,}",
            "Original source": f"{verifier_result.source_width_px:,} × {verifier_result.source_height_px:,}",
            "Final file": f"{verifier_result.final_width_px:,} × {verifier_result.final_height_px:,}",
            "Original megapixels": verifier_result.source_megapixels,
            "Final megapixels": verifier_result.final_megapixels,
            "Effective DPI at A1": verifier_result.effective_dpi,
            "Processing method": verifier_result.processing_method,
            "Classification": verifier_label,
            "Meets 240 DPI minimum": "Yes" if verifier_result.passes_minimum_240_dpi else "No",
            "Meets 300 DPI target": "Yes" if verifier_result.passes_target_300_dpi else "No",
            "Native A1 source": "Yes" if verifier_result.native_target_met else "No",
            "Aspect-ratio difference": f"{verifier_result.aspect_ratio_difference_percent:.3f}%",
        }
        st.table([{"Check": key, "Result": value} for key, value in report_rows.items()])

        for verifier_warning in verifier_result.warnings:
            st.warning(verifier_warning)

        st.download_button(
            "Download verification report JSON",
            data=json.dumps(verifier_result.to_dict(), indent=2),
            file_name="a1_print_verification_report.json",
            mime="application/json",
            key="v32_download_verification_report",
            use_container_width=True,
        )
    except Exception as verifier_exc:
        st.error(f"The artwork could not be verified: {verifier_exc}")
