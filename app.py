...
def make_recipe(destinations, horizon_y, label_settings):
    ...
    return {
        "recipe_version": "1.2.2",
        "build_name": "Destination Labels + Roof-Mounted Country Flags",
        "style_preset": style,
...
def make_guide(recipe):
    ...
    for index, item in enumerate(recipe["destinations"]):
        ...
        # This point is used when "roof-mounted" flagpoles are selected.
        landmark_anchor_x = int(x + w / 2)
        landmark_anchor_y = int(horizon_y - 245)
        ...
        for building_index in range(building_count):
            ...
            # Use the centre building as the visual landmark anchor.
        ...
        if labels_enabled and item.get("destination_label"):
            ...
            if label.get("include_flag") and label.get("country_name"):
                ...
                if flag_anchor == "landmark_roof":
                    # Pole grows directly from the roof point.
                    pole_bottom = landmark_anchor_y + 2
                    pole_top = max(label_y + text_h + 34, pole_bottom - 105)
                    ...
                    # Small mounting foot makes the physical connection clear.
                    draw.ellipse(
                        (
                            landmark_anchor_x - 5,
                            pole_bottom - 4,
...
st.title("🎨 Place Blend Recipe Builder — Roof-Mounted Flags")
st.write(
