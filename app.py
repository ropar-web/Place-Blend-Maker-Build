...
def make_recipe(destinations, horizon_y, label_settings, canvas_settings, art_settings):
    ...
    return {
        ...
        "build_name": "Multi-Style Place Blender 2.0",
        "style_preset": style,
...
def make_prompt(recipe):
    ...
    style_direction = ART_STYLE_PRESETS.get(style_name, ART_STYLE_PRESETS["Premium Travel Poster"])["prompt"]
    render_rules = STYLE_RENDER_RULES.get(style_name, STYLE_RENDER_RULES["Premium Travel Poster"])
    ...
    for item in recipe["destinations"]:
        destination_lines.append(
            ...
            f"Include {joined(item['secondary_elements'])}. Preserve {joined(item['important_features'])}."
        )
        if labels_enabled and item.get("destination_label"):
            label = item["destination_label"]
            approved_labels.append(f'- "{label["text"]}" {label["placement"]}')

    label_block = ""
    if labels_enabled:
        label_block = f"""
APPROVED DESTINATION LABELS ONLY:
{chr(10).join(approved_labels)}
Use {label_style['font_style']} in {label_style['colour']}. Spell every label exactly. Do not add any other words.
"""
        if label_style.get("include_country_flags"):
            label_block += "Use only the correct national flag for each destination. Keep flags clearly attached or positioned exactly as selected; never float flagpoles in the sky.\n"

    personalisation = "Do not add any wording unless destination labels were explicitly enabled."
    if title_text or subtitle_text:
        ...
        personalisation = "Include only the requested personalisation — " + "; ".join(requested) + ". Spell it exactly."

    blend_conflict_rule = {
        "Poster Grid": "Keep each destination in its own separate tile. Do not create a seamless panorama.",
        "Map Connection": "Use map forms only. Do not turn the result into a landmark panorama.",
        "Main Place + Memory Inset": "Use one dominant main scene plus clearly subordinate inset artwork.",
        "Side by Side": "Keep distinct destination zones while maintaining one consistent selected art style.",
        "Layered Scene": "Use depth layers, not panels or tiles.",
        "Smooth Blend": "Create one continuous scene with no panels or hard divider."
    }.get(blend_name, "Follow the selected blend structure exactly.")

    reference_rule = (
        "Use uploaded place photographs only for landmark identity, proportions and local details. "
        "Use an uploaded style reference only when it matches the selected style. Ignore any conflicting style cues from other references. "
        "Never average or mix the visual styles of multiple reference images."
    )

    return f"""Create one high-quality personalised place artwork.
...
Do not combine, average, layer or borrow characteristics from any other available art style. The entire output must use only the selected style from edge to edge.
...
QUALITY RULES:
Keep every named place recognisable and geographically appropriate. Preserve defining architecture and avoid duplicated or invented major landmarks. Maintain one coherent composition and one consistent selected style.

DO NOT INCLUDE:
Unrequested text, logos, watermarks, unrelated landmarks, mixed art styles, contradictory rendering methods, warped architecture, duplicate landmarks, random fantasy buildings, childish clipart or glossy 3D toy styling.
"""
