...
"""
    ...
    if title_text or subtitle_text:
        ...
        personalisation = "Include only the requested personalisation — " + "; ".join(requested) + ". Spell it exactly and integrate it elegantly."

    return f"""Create one high-quality personalised place artwork combining the listed destinations.

Use any attached place reference images to improve landmark accuracy. Use an attached approved style reference for overall visual character only; do not copy unrelated buildings or its exact composition. Use the rendered layout guide as a composition guide where relevant.

SELECTED STYLE — {style_name.upper()}:
{style_direction}

SELECTED BLEND — {blend_name.upper()}:
{blend_direction}
...
BLENDING:
Follow the selected blend method exactly. Keep the overall result coherent, balanced and intentionally designed.

ACCURACY:
Follow the selected accuracy level. Make every named place geographically appropriate and preserve its defining identity.
