...
def make_scenic_recipe(destinations, horizon_y, label_settings, canvas, art_settings):
    ...
    # The bold retro family supports three locked surface treatments.
    # This changes the rendering language consistently across the JSON and prompt
    # instead of mixing a watercolour request into the crisp graphic preset.
    if style_name == "Bold Colour Retro Travel Poster":
        poster_finish = art_settings.get("poster_finish", "Crisp graphic poster")
        if poster_finish == "Watercolour retro poster":
            render["finish"] = (
                "A genuine hand-painted watercolour destination artwork arranged in a bold retro travel-poster layout. "
                "Use layered transparent washes, visible cold-pressed paper grain, wet-on-wet sky passages, pigment blooms, "
                "soft colour bleeding, dry-brush accents and naturally uneven hand-painted edges. Keep the named landmark "
                "recognisable, but do not render it as crisp vector art, screen printing or flat digital colour blocks."
            )
            render["palette"] = (
                "Luminous transparent watercolour pigments in turquoise, cyan, teal, sap green, leaf green, sunshine yellow, "
                "coral, rose and indigo, with natural pigment granulation, layered colour variation, preserved white paper "
                "highlights and a clean white print margin."
            )
            render["lighting"] = (
                "Bright daylight painted through translucent washes, softly diffused shadows, reserved paper highlights, "
                "watery sky transitions and gentle tonal layering rather than hard-edged digital poster shadows."
            )
            render["exclude"] = (
                "flat vector fills, screen-print-like hard edges, rigid geometric colour blocks, ultra-crisp digital contours, "
                "glossy poster rendering, heavy opaque oil-paint texture, photorealism, 3D rendering, childish clipart, "
                "map silhouettes, ink-outline sketch styling or mixed illustration styles"
            )
        elif poster_finish == "Painted graphic poster":
            render["finish"] = (
                "Contemporary bold-colour retro destination-poster finish with clean illustrated shapes softened by "
                "subtle hand-painted edges, restrained brush texture, simplified accurate landmarks and polished layered depth."
            )
            render["palette"] = (
                "Highly saturated coordinated turquoise, teal, green, yellow, coral, pink and deep navy with subtle "
                "paint variation and a clean white print margin."
            )
            render["lighting"] = "Bright graphic daylight with clean poster shadows and gentle painted tonal variation."
            render["exclude"] = (
                "pale washed-out watercolour, loose ink sketching, muddy muted vintage grading, photorealism, "
                "3D rendering, childish clipart, map silhouettes or mixed illustration styles"
            )
    ...
    if style_name == "Bold Colour Retro Travel Poster":
        poster_finish = art_settings.get("poster_finish", "Crisp graphic poster")
        finish_line = {
            "Crisp graphic poster": "use crisp saturated hard-edged colour shapes with screen-print-like clarity",
            "Painted graphic poster": "use clean graphic shapes with subtle hand-painted edges and restrained brush texture",
            "Watercolour retro poster": "render the entire scenic artwork as genuine hand-painted watercolour with visible paper grain, wet-on-wet washes, pigment blooms, soft bleeding edges and dry-brush detail, while retaining only the clean retro poster layout, border and typography",
        }.get(poster_finish, "use crisp saturated hard-edged colour shapes with screen-print-like clarity")
        composition_by_style[style_name] = [
            "one consistent bold-colour retro destination-poster composition",
            "a dominant recognisable landmark with simplified colourful architecture and strong graphic depth",
            finish_line,
            "optional light foliage framing and a clean white poster margin",
            "when labels are enabled, place the large destination name and smaller country subtitle beneath the artwork rather than in the sky",
            "use the selected blend structure exactly"
        ]
    ...
    return {
        ...
        "build_name": "Multi-Style Place Blender 2.4",
...
# ── SCENIC POSTER / RETRO / WATERCOLOUR QUESTIONS ─────────────────────────
    ...
    if style_name == "Bold Colour Retro Travel Poster":
        st.subheader("3. Bold-colour poster treatment")
        poster_finish = st.selectbox(
            "Poster rendering finish",
            ["Crisp graphic poster", "Painted graphic poster", "Watercolour retro poster"],
            index=0,
            help="Choose crisp flat poster shapes, a lightly painted graphic finish, or a genuine hand-painted watercolour scene that keeps only the retro poster layout, white margin and title treatment."
        )
        colour_strength = st.selectbox("Colour strength", ["Bold", "Extra bold", "Balanced"], index=0)
        foliage_framing = st.selectbox(
            "Decorative foliage framing",
            ["Light top foliage framing", "Top foliage only", "Top and bottom foliage", "Subtle corner foliage", "No foliage"],
            index=0
        )
        white_border = st.checkbox("Add a clean white poster border", value=True)
        title_position = st.selectbox("Destination title position", ["Below the artwork in the white margin", "Inside the lower artwork area", "No destination title"])
        country_subtitle = st.checkbox("Show smaller country name below each destination title", value=True, disabled=(title_position == "No destination title"))
        extra_art_settings.update({
            "poster_finish": poster_finish,
            "colour_strength": colour_strength,
            "foliage_framing": foliage_framing,
            "white_poster_border": white_border,
            "destination_title_position": title_position,
            "country_subtitle": country_subtitle,
        })
        if title_position != "No destination title":
            include_labels = True
            label_placement = "large destination name below the artwork in the clean white poster margin, with a smaller country subtitle directly beneath" if title_position.startswith("Below") else "large destination name integrated inside the lower artwork area"
...
label_settings = {
    ...
    "font_style": "clean modern uppercase sans-serif poster lettering" if style_name == "Bold Colour Retro Travel Poster" else "elegant classic high-contrast serif lettering inspired by premium travel prints",
    "colour": label_colour,
