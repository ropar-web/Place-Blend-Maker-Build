...
st.set_page_config(
    page_title="Place Blend Recipe Builder 3.1",
    page_icon="🎨",
    layout="wide",
)
...
def match_destination(text):
    ...
    # Match the most specific alias, not whichever preset happens to appear first.
    # Exact matches always win. Partial matches are ranked by the longest matching
    # alias so a detailed place name cannot be overridden by a broader city name.
    exact_matches = []
    partial_matches = []
    for key, preset in DESTINATIONS.items():
        ...
        for candidate in candidates:
            ...
            if query == candidate_norm:
                exact_matches.append((len(candidate_norm), key, preset))
            elif candidate_norm and (candidate_norm in query or query in candidate_norm):
                partial_matches.append((len(candidate_norm), key, preset))

    matches = exact_matches or partial_matches
    if matches:
        _, key, preset = max(matches, key=lambda match: match[0])
...
def make_scenic_recipe(destinations, horizon_y, label_settings, canvas, art_settings):
    ...
    return {
        ...
        "build_name": "Multi-Style Place Blender 3.2.3",
...
def make_geography_guard(destinations):
    """Build strict place-specific exclusions so scenic styling cannot alter geography."""
    all_have_water = all(bool(item.get("water", False)) for item in destinations)
    any_have_water = any(bool(item.get("water", False)) for item in destinations)
    no_water_names = [item.get("display_name", item.get("typed_name", "destination")) for item in destinations if not item.get("water", False)]

    rules = [
        "GEOGRAPHIC REALITY OVERRIDES DECORATIVE STYLE.",
        "Render the exact typed destination and its named landmark; never substitute a different city, country, landmark or generic holiday location.",
        "Do not invent castles, palaces, fantasy towers, domes, bridges, monuments, mountains or waterfronts that are not listed for that destination.",
        "Do not import scenery from another destination preset merely because it looks attractive.",
        "Keep every building recognisably grounded in the stated local architecture; decorative styling may simplify surfaces but must not change structural identity.",
    ]

    if not any_have_water:
        rules.append("NO WATER ANYWHERE: no sea, ocean, harbour, river, lake, lagoon, canal, beach, shoreline, boats, islands, waterfalls or reflections resembling water.")
    elif not all_have_water:
        rules.append("Water may appear ONLY inside destination zones explicitly marked as having water. Keep all non-water destinations completely dry and urban/inland as specified.")
        if no_water_names:
            rules.append("The following destination zones must contain no water: " + ", ".join(no_water_names) + ".")

    for item in destinations:
        name = item.get("display_name", item.get("typed_name", "Destination"))
        terrain = item.get("terrain", "geographically appropriate terrain")
        landmark = item.get("main_landmark", "the stated landmark")
        water_rule = "water is permitted only as described" if item.get("water", False) else "strictly no water"
        rules.append(f"{name}: show {landmark}; terrain must remain {terrain}; {water_rule}.")
...
def make_scenic_prompt(recipe):
    ...
    geography_guard = make_geography_guard(recipe["destinations"])
    ...
    for item in recipe["destinations"]:
        ...
        exact_typed = item.get("typed_name", item["display_name"])
        water_instruction = (
            "Water is geographically required only where described for this destination."
            if item.get("water", False)
            else "This destination is dry/urban/inland for this artwork: include absolutely no water, shoreline, boats or islands."
        )
        destination_lines.append(
            f"EXACT REQUEST: {exact_typed}. Render {item['display_name']} on the {item['position']}: feature {item['main_landmark']}. "
            f"Include only these supporting elements where appropriate: {joined(item.get('secondary_elements', []))}. "
            f"Preserve {joined(item.get('important_features', []))}. Terrain: {item.get('terrain', 'geographically accurate terrain')}. "
            f"{water_instruction}{viewpoint_text}"
        )
...
STRICT GEOGRAPHY AND ANTI-HALLUCINATION RULES:{geography_guard}
